...
/**
 * Interface for Resources which provide CRUDS end points. The implementation of this is not meant
 * to be exposed as an actual end point though. It is meant to be delegated to by an exposed
 * resource.
 * 
...
public interface CrudsResource<T extends DomainObject> extends Resource {
  ...
  @ApiResponses(value = {@ApiResponse(code = 404, message = "Not found"),
      @ApiResponse(code = 406, message = "Accept Header not supported")})
  @ApiOperation(hidden = true, value = "Delete DomainObject", code = HttpStatus.SC_OK,
   ...
   *         {@link gov.ca.cwds.rest.api.domain.ApiResponse} for the newly create
   *         {@link DomainObject}
   */
  @POST
  ...
  @ApiOperation(hidden = true, value = "Create DomainObject", response = Object.class,
      code = HttpStatus.SC_CREATED, responseHeaders = @ResponseHeader(name = "Location",
          description = "Link to the newly created object", response = Object.class))
  ...
  @Consumes(value = MediaType.APPLICATION_JSON)
  @ApiOperation(hidden = true, value = "Update DomainObject", code = HttpStatus.SC_NO_CONTENT,
      response = Object.class)
