...
public class GenealogistTest {
    ...
    public void setup(){
        ...
        sonClient = new ClientEntityBuilder().setId("1").setCommonFirstName("Charlie").setSensitivityIndicator("R").build();
        dadClient = new ClientEntityBuilder().setId("1").setCommonFirstName("Ralph").setCommonLastName("Foo").setSensitivityIndicator("S").build();
        momClient = new ClientEntityBuilder().setId("2").setCommonFirstName("Rene").setCommonLastName("Foo").build();
        sisterClient = new ClientEntityBuilder().setId("3").setCommonFirstName("Rebecca").setCommonLastName("Foo").build();
        brotherClient = new ClientEntityBuilder().setId("4").setCommonFirstName("Ricky").setCommonLastName("Foo").build();
        ...
        RelationshipWrapper client1Mother = new RelationshipWrapper("123", sonClientId, momClientId, "Charlie", "Foo", "Rene", "Foo", MONTHER_SON, SON_MOTHER);
        RelationshipWrapper client1Father = new RelationshipWrapper("432", sonClientId, dadClientId, "Charlie", "Foo", "Ralph", "Foo", FATHER_SON, SON_FATHER);
        relationships = Arrays.asList(client1Mother, client1Father);

        RelationshipWrapper client1Brother = new RelationshipWrapper("123", brotherClientId, sonClientId, "Ricky", "Foo", "Charlie", "Foo", BROTHER_BROTHER, BROTHER_BROTHER);
        RelationshipWrapper client1Sister = new RelationshipWrapper("432", sisterClientId, sonClientId, "Rebecca", "Foo", "Charlie", "Foo", BROTHER_SISTER, SISTER_BROTHER);
    ...
    public void givenAClientIdAndRelationshipWhenOnlySecondaryRelationsExistsThenWeShouldGetRelationships(){
        Relationship relationship = genealogist.buildRelationships(secondaryOnlyRelationships,sonClientId);
        ArrayList<RelationshipTo> orderedRelationships = convertToOrderedRelationships(relationship);

        assertEquals(brotherClient.getCommonFirstName(), orderedRelationships.get(1).getRelatedFirstName());
        assertEquals(brotherClient.getCommonLastName(), orderedRelationships.get(1).getRelatedLastName());
        ...
        assertEquals(sisterClient.getCommonFirstName(), orderedRelationships.get(0).getRelatedFirstName());
