...
public class EducationProviderContactTransformerTest {
  ...
  public void testConvertExpectdVsActual() throws Exception {
    ...
    Set<AddressIntakeApi> addresses = new HashSet<>(
        Arrays.asList(new AddressIntakeApi("ED_PVDRT", "fk12345678", "streetNumber streetName",
            "Sacramento", "CA", "99999-0", null, addressLegacyDescriptor)));
    Set<PhoneNumber> phoneNumbers = new HashSet<>(Arrays.asList(new PhoneNumber(null, "0", null)));
    ParticipantIntakeApi expected =
        new ParticipantIntakeApi(null, null, "1234567ABC", legacyDescriptor, "Firstname", "Middle",
            "Lastname", "Esq", null, null, null, null, null, new LinkedList<>(), null, null, null,
