...
public class AllegationResourceImplTest {
	...
	private static final String ROOT_RESOURCE = "/" + ApiPoc.RESOURCE_ALLEGATION + "/";
	...
	@Before
	public void setup() {
		when(serviceEnvironment.getService(AllegationService.class, ApiPoc.Version.JSON_VERSION_1.getMediaType()))
	...
	public void getDelegatestoCrudsResource() throws Exception {
		...
		verify(crudsResource, times(1)).get(toVerify.getId(), ApiPoc.MEDIA_TYPE_JSON_V1);
	...
	public void updateDelegatestoCrudsResource() throws Exception {
		Allegation toCreate = MAPPER.readValue(fixture("fixtures/legacy/Allegation/valid/valid.json"),
				Allegation.class);
		resources.client().target(ROOT_RESOURCE).request().accept(ApiPoc.Version.JSON_VERSION_1.getMediaType())
				.put(Entity.entity(toCreate, ApiPoc.MEDIA_TYPE_JSON_V1));
	...
	public void createValidatesAllegation() throws Exception {
		...
		assertThat(resources.client().target(ROOT_RESOURCE).request().accept(ApiPoc.Version.JSON_VERSION_1.getMediaType())
				.post(Entity.entity(toCreate, ApiPoc.MEDIA_TYPE_JSON_V1)).getStatus(), is(equalTo(422)));
	}
	...
	public void updateValidatesAllegation() throws Exception {
		Allegation toCreate = MAPPER.readValue(fixture("fixtures/legacy/Allegation/invalid/id/missing.json"),
				Allegation.class);
		assertThat(resources.client().target(ROOT_RESOURCE).request().accept(ApiPoc.Version.JSON_VERSION_1.getMediaType())
				.put(Entity.entity(toCreate, ApiPoc.MEDIA_TYPE_JSON_V1)).getStatus(), is(equalTo(422)));
	}
}
