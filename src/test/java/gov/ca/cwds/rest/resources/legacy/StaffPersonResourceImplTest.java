...
public class StaffPersonResourceImplTest {
	...
	public void setup() throws Exception {
		...
		when(serviceEnvironment.getService(StaffPersonService.class, ApiPoc.Version.JSON_VERSION_1.getMediaType())).thenReturn(staffPersonService);
		when(crudsResource.get( eq(valid.getId()), eq(ApiPoc.Version.JSON_VERSION_1.getMediaType()))).thenReturn(Response.ok(valid).build());
	}
