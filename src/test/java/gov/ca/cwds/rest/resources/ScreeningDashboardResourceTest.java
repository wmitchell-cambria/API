...
public class ScreeningDashboardResourceTest extends IntakeBaseTest{
  
  private  final static ScreeningService service = mock(ScreeningService.class);
  
  @ClassRule
  public static final ResourceTestRule resources = ResourceTestRule.builder()
  .addResource(new ScreeningDashboardResource(service))
  .build();
  ...
  public void shouldCallScreeningsEndPoint() throws Exception{
	...
	when(service.findScreeningDashboard(screeningDecisionDetail, screeningDecision, referralId)).thenReturn(screeningDashboardList);
	
	resources.client()
	.target("/screenings")
	.queryParam("screening_decision_detail", sdd)
	.queryParam("screening_decision", sd)
	.queryParam("referral_id", referralId)
	.request()
