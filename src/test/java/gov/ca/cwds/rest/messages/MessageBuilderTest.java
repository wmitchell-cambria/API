...
public class MessageBuilderTest {
  ...
  public void shouldContainValidationErrorMessageForDomainObject(){
    ...
    Address address = Address.createWithDefaults(new gov.ca.cwds.rest.api.domain.Address(
                                                    null,null,"1 main", longWelshCityName , null, 123435, "type"),
                                                 new Short("1234"));
    Validator validator = Validation.buildDefaultValidatorFactory().getValidator();

    builder.addDomainValidationError(validator.validate(address));
    ErrorMessage message = builder.getMessages().get(0);
    assertEquals("Expected Error to be formated with Resource Error message", "size must be between 0 and 20", message.getMessage() );
    assertEquals("Expected Error to have default validation type ", ErrorType.VALIDATION, message.getType() );
  }
}