...
public class ReferralTest {
	...
	public void domainReferralLastUpdateConstructorTest() throws Exception {
		gov.ca.cwds.rest.api.domain.legacy.Referral domain = new gov.ca.cwds.rest.api.domain.legacy.Referral(id,
				...
				referredToResourceType, responseDeterminationDate, responseDeterminationTime, responseRationaleText,
				screenerNoteText, specificsIncludedCode, sufficientInformationCode, unfoundedSeriesCode,
				linkToPrimaryReferralId, allegesAbuseOccurredAtAddressId, firstResponseDeterminedByStaffPersonId,
				primaryContactStaffPersonId, countySpecificCode, specialProjectReferralIndicator, zippyCreatedIndicator,
				homelessIndicator, familyRefusedServicesIndicator, firstEvaluatedOutApprovalDate, responsibleAgencyCode,
		...
		assertThat(persistent.getDrmsAllegationDescriptionDoc(), is(equalTo(drmsAllegationDescriptionDoc)));
		...
		assertThat(persistent.getFiledSuspectedChildAbuseReporttoLawEnforcementIndicator(), is(equalTo("Y")));
		...
		assertThat(persistent.getMandatedCrossReportReceivedDate(), is(equalTo(df.parse(mandatedCrossReportReceivedDate))));
		...
		assertThat(persistent.getResponseDeterminationDate(), is(equalTo(df.parse(responseDeterminationDate))));
		assertThat(persistent.getResponseDeterminationTime(), is(equalTo(tf.parse(responseDeterminationTime))));
		...
		assertThat(persistent.getAllegesAbuseOccurredAtAddressId(), is(equalTo(allegesAbuseOccurredAtAddressId)));
		assertThat(persistent.getFirstResponseDeterminedByStaffPersonId(), is(equalTo(firstResponseDeterminedByStaffPersonId)));
		assertThat(persistent.getPrimaryContactStaffPersonId(), is(equalTo(primaryContactStaffPersonId)));
		...
		assertThat(persistent.getFirstEvaluatedOutApprovalDate(), is(equalTo(df.parse(firstEvaluatedOutApprovalDate))));
		assertThat(persistent.getResponsibleAgencyCode(), is(equalTo(responsibleAgencyCode)));
		assertThat(persistent.getLimitedAccessGovtAgencyType(), is(equalTo(limitedAccessGovtAgencyType)));
