...
import gov.ca.cwds.data.persistence.auth.CmsUserAuthPrivilege.UnitAuthority;
...
public class UserAuthorizationTest {
  ...
  private StaffUnitAuthority staffUnitAuthority = new StaffUnitAuthority("Unitwide Read", "ABC123",
      "Sacramento", UnitAuthority.UNIT_AUTH_SUPERVISOR.getDescription());
  private StaffAuthorityPrivilege authorityPrivilege = new StaffAuthorityPrivilege(
      "Countywide Read", "P", "Placer", UnitAuthority.UNIT_AUTH_SUPERVISOR.getDescription());
  private ImmutableSet<StaffUnitAuthority> testuserUnitAuthority =
      ImmutableSet.<StaffUnitAuthority>builder().add(staffUnitAuthority).build();
  private ImmutableSet<StaffAuthorityPrivilege> testuserAuthorityPrivilege =
      ImmutableSet.<StaffAuthorityPrivilege>builder().add(authorityPrivilege).build();

  @ClassRule
  public static final ResourceTestRule resources =
      ResourceTestRule.builder().addResource(mockedUserAuthorizationResource).build();
  ...
  @Before
  public void setup() {
    when(mockedUserAuthorizationResource.create(eq(validUserAuthorization)))
  ...
  public void jsonCreatorConstructorTest() throws Exception {
    ...
    UserAuthorization domain = new UserAuthorization(userId, staffPersonId, socialWorker,
        supervisor, overrideAuthority, testuserAuthorityPrivilege, testuserUnitAuthority);
  ...
  private UserAuthorization validUserAuthorization() {
    StaffUnitAuthority staffUnitAuthority = new StaffUnitAuthority("Unitwide Read", "ABC123",
        "Sacramento", UnitAuthority.UNIT_AUTH_SUPERVISOR.getDescription());
    StaffAuthorityPrivilege authorityPrivilege = new StaffAuthorityPrivilege("Countywide Read", "P",
        "Placer", UnitAuthority.UNIT_AUTH_SUPERVISOR.getDescription());
